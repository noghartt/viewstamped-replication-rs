...
fn node_tag(node: &NodeKind) -> String {
    let NodeId(id) = match node {
        NodeKind::Client(id) => return format!("C{}", id.0),
        NodeKind::Replica(id) => *id,
    };
    format!("R{id}")
}
...
impl<Input: fmt::Debug, Output: fmt::Debug> fmt::Display for History<Input, Output> {
